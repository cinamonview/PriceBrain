...
## 13. 개발 에이전트 상호작용 및 언어 규칙

개발 에이전트(AI 어시스턴트)는 사용자와 상호작용할 때 다음 규칙을 준수한다.

### 13.1 사용자 응답 언어 규칙
- **모든 설명, 진행 상황, 분석 결과, 최종 보고서 및 질문/선택지는 한국어로 작성한다.**
- 영어 전용 선택지(`Yes/No`, `Approve/Reject`, `Continue/Cancel`, `Proceed/Stop` 등)를 사용자에게 직접 제시하지 않는다.
- 반드시 명확한 한국어 선택지를 제공한다 (예: `계속 진행 / 중단`, `1. 계속 진행 2. 중단`).

### 13.2 기술 용어 표기 원칙
- 코드, 파일 경로, Git 브랜치/커밋, API 엔드포인트, 에러 원문 등 기술 식별자는 영어 원문을 유지하되, 이에 대한 설명은 한국어로 작성한다.

### 13.3 사용자 승인 요청 규칙
- 사용자의 확인이나 승인이 필요한 경우 실행 목적, 필요성, 변경 범위, 가능한 선택지를 한국어로 제시한다.
